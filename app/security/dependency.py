...
from app.workspace_service.schema import WorkSpaceMemberRespond
...
def require_role(minimum_role:str):
    def dependency(wk_id:int,user:User=Depends(get_current_user),db:Session=Depends(get_db)):
        get = db.query(WorkSpaceMember).filter(WorkSpaceMember.workspace_id==wk_id,WorkSpaceMember.user_id == user.id).first()
        if not get:
            raise HTTPException(
                status_code=403,detail="Not participant of this workspace"
            )
        if role_rank[get.role]>=role_rank[minimum_role]:
            return WorkSpaceMemberRespond.model_validate(get)
        else:
